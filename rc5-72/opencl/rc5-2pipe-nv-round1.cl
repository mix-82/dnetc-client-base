//CORENAME=ocl_rc572_2pipe_nv_round1_src
...
__kernel void ocl_rc572_2pipe_nv_round1( __constant const uint * rc5_72unitwork, volatile __global uint * outbuf)
{
  uint2 L[3];
  uint2 S[26];
  uint2 A, B;
  ...
  if(l1_t2 < l1_t1)
  {
    ...
    l0_t += 1;
    L[0] = (uint2)ROTL1(0xBF0A8B1D + SWAP(l0_t), 0x1d);
    S[1] = (uint2)ROTL1(L[0].x + 0xBF0A8B1D + 0x5618cb1c, 3u);
  }
  L[1].x = SWAP(l1_t2);
  
  S[0] = (uint2)0xBF0A8B1D;
  t.x = S[1].x + L[0].x;
  L[1] = (uint2)ROTL1(L[1].x + t.x, t.x);
  L[2].y = L[2].x + 1;

  //ROUND1( 2,  1, 1, 2);
  S[2] = (uint2)ROTL1(S[1].x + (P + 2 * Q) + L[1].x, 3u);
  t = (uint2)(S[2].x + L[1].x);
  L[2] = ROTL(L[2] + t, t);

  //ROUND1( 3,  2, 2, 0);
  S[3] = ROTL3((uint2)(S[2].x + (P + 3 * Q)) + L[2]);
  t = S[3] + L[2];
  L[0] = ROTL(L[0] + t, t);
